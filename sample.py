...
def prior_sample(evidence, nodes, values, probability, n, graph, graph2):
    sort_vertex = topologicalSort(graph)
    samples = []
...
length = len([name for name in os.listdir('.\inputs')])

for o in range(1, length + 1):
    path = os.path.join(".\inputs", str(o) + "\input.txt")
    file = open(path, 'r')
    n = int(file.readline())
    names = {}
    probability = []
    ...
    path = os.path.join(".\inputs", str(o) + "\q_input.txt")
    file = open(path, 'r')
    data = file.readline()
    x = json.loads(data)
    ...
    for query in x:
        ...
        result_string += str(result_real) + " " + str(abs(result_prior_sample - result_real)) + " " + str(abs(
            result_rejection_sample - result_real)) + " " + str(
            abs(result_likelihood_sample - result_real)) + " " + str(abs(
            result_gibbs_sample - result_real)) + "\n"
        prior.append(abs(result_prior_sample - result_real))
        reject.append(abs(result_rejection_sample - result_real))
        likelihood.append(abs(result_likelihood_sample - result_real))
        gibbs.append(abs(result_gibbs_sample - result_real))

    file.close()

    path = os.path.join(".\output", str(o) + ".txt")
    file = open(path, 'w')
